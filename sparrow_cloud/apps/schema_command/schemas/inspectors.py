...
from ..schemas.compat import coreapi, coreschema, uritemplate
from ..schemas import incompatible_settings
from rest_framework.utils import formatting

from ..schemas.patch import patch_paginator, patch_filter_backend
...
class AutoSchema(ViewInspector):
    ...
    def get_link(self, path, method, base_url, extract_detail=False):
        """extract_detail控制是否提取输入输出参数"""
        fields = self.get_path_fields(path, method)
        if extract_detail is True:
    ...
    def _get_description_section(self, view, header, description):
        ...
        coerce_method_names = incompatible_settings.SCHEMA_COERCE_METHOD_NAMES
    ...
    def get_filter_fields(self, path, method):
        ...
        for filter_backend in self.view.filter_backends:
            _filter_backend = filter_backend()
            # 为filter_backend 添加上get_schema_fields
            patch_filter_backend(_filter_backend)
            fields += _filter_backend.get_schema_fields(self.view)
        return fields
